...
class player:
    ...
    def update(self):
        ...
        if Level.level == 0:
            ...
            pygame.draw.circle(gameDisplay,white,[self.pos_x,self.pos_y-player1.size], self.size//2)
...
def gameover():
    ...
    pygame.mixer.music.load('Roundabout by Yes (to be continued meme music) (256  kbps).mp3')
...
while running:
    ...
    if player1.size < 1:
        gameover()
    if player1.size >= 20 and Level.level ==0:
        Level.level = Level.level + 1
        player1.size = 15
    if player1.size >= 30 and Level.level ==1:
        win()
    player1.update()
    pygame.display.update()
    clock.tick(FPS)